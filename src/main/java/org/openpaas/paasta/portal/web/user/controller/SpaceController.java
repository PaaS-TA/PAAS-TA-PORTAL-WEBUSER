...
public class SpaceController extends Common {
    ...
    public Space getSpaceSummary(@RequestBody Space space) {
        try {
            Catalog catalogParameter = new Catalog();
            catalogParameter.setSearchTypeUseYn("Y");
            Map<String, Object> servicePackCategory = commonService.procRestTemplate("/catalog/getServicePackCatalogList", HttpMethod.POST, catalogParameter, null);

            List<Map> list = (List) servicePackCategory.get("list");
            ...
            List<Service> serviceList = rspSpace.getServices();

            for (int i = 0; i < serviceList.size(); i++) {
                for (Map catalog : list) {
                    String catalogServicePackName = null;
                    String catalogDashboardUseYn = null;
                    String catalogappBindYn = null;
                    String catalogappBindParameter = null;
                    String catalogserviceParameter = null;
                    if (catalog != null && catalog.get("servicePackName") instanceof String) {
                        catalogServicePackName = (String) catalog.get("servicePackName");
                        catalogDashboardUseYn = (String) catalog.get("dashboardUseYn");
                        catalogappBindYn = (String) catalog.get("appBindYn");
                        catalogappBindParameter = (String) catalog.get("app_bind_parameter");
                        catalogserviceParameter = (String) catalog.get("parameter");
                        if(serviceList.get(i).getServiceLabel() != null) {
                            if (serviceList.get(i).getServiceLabel().trim().equalsIgnoreCase(catalogServicePackName.trim())) {
                                serviceList.get(i).setDashboardUseYn(catalogDashboardUseYn);
                                serviceList.get(i).setAppBindYn(catalogappBindYn);
                                serviceList.get(i).setAppBindParameter(catalogappBindParameter);
                                serviceList.get(i).setParameter(catalogserviceParameter);
    ...
    public List getUsersForSpaceRole(@RequestBody Map body) {
        ...
        body.put("userId", commonService.getUserId());

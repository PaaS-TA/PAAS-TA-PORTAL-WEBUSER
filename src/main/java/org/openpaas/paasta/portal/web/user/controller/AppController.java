...
import org.openpaas.paasta.portal.web.user.common.TimeConverter;
import org.openpaas.paasta.portal.web.user.model.App;
import org.openpaas.paasta.portal.web.user.model.AppMonitoringChart;
...
public class AppController extends Common {
     ...
     * @return ModelAndView model
     */
    @RequestMapping(value = {"/app/statusChart/{key}"}, method = RequestMethod.GET)
    public ModelAndView appStatusChart(@PathVariable int key) {
        ModelAndView model = new ModelAndView();
        model.addObject("key", key);
        model.setViewName("/app/statusChart");
        return model;
    }
     ...
     * @return Map
     */
    @RequestMapping(value = {"/app/getCpuUsage"}, method = RequestMethod.POST)
    @ResponseBody
    public Map getAppMonitoringCpuUsage(@RequestBody AppMonitoringChart appMonitoringChart,
                                        @RequestParam(value = "defaultTimeRange") String defaultTimeRange,
                                        @RequestParam(value = "groupBy") String groupBy) throws ParseException {
        List<Map> dataList = new ArrayList<Map>();

        if (appMonitoringChart.getIdx().toUpperCase().indexOf(",") >= 0) {
            String[] idx = appMonitoringChart.getIdx().split(",");
            for (String id : idx) {
                Map data = new HashMap();
                ResponseEntity<Map> result = commonService.procRestTemplate("/app/" + appMonitoringChart.getGuid() + "/" + id + "/cpuUsage?defaultTimeRange=" + defaultTimeRange + "&groupBy=" + groupBy, HttpMethod.GET, null, null, Map.class);
            ...
            ResponseEntity<Map> result = commonService.procRestTemplate("/app/" + appMonitoringChart.getGuid() + "/" + appMonitoringChart.getIdx() + "/cpuUsage?defaultTimeRange=" + defaultTimeRange + "&groupBy=" + groupBy, HttpMethod.GET, null, null, Map.class);
            Map body = result.getBody();
            Map data = new HashMap();

            data.put("name", appMonitoringChart.getIdx());
            data.put("data", body);
            dataList.add(data);
        }
        ...
        returnMap.put("data", timeConverter.timeConverter("cpu_", dataList));
        return returnMap;
    }
     ...
     * @return Map
     */
    @RequestMapping(value = {"/app/getMemoryUsage"}, method = RequestMethod.POST)
    @ResponseBody
    public Map getAppMonitoringMemoryUsage(@RequestBody AppMonitoringChart appMonitoringChart,
                                           @RequestParam(value = "defaultTimeRange") String defaultTimeRange,
        ...
        if (appMonitoringChart.getIdx().toUpperCase().indexOf(",") >= 0) {
            ...
            for (String id : idx) {
                Map data = new HashMap();
                ResponseEntity<Map> result = commonService.procRestTemplate("/app/" + appMonitoringChart.getGuid() + "/" + id + "/memoryUsage?defaultTimeRange=" + defaultTimeRange + "&groupBy=" + groupBy, HttpMethod.GET, null, null, Map.class);
                Map body = result.getBody();
            ...
            ResponseEntity<Map> result = commonService.procRestTemplate("/app/" + appMonitoringChart.getGuid() + "/" + appMonitoringChart.getIdx() + "/memoryUsage?defaultTimeRange=" + defaultTimeRange + "&groupBy=" + groupBy, HttpMethod.GET, null, null, Map.class);
            Map body = result.getBody();
            Map data = new HashMap();
        ...
        TimeConverter timeConverter = new TimeConverter();
        returnMap.put("data", timeConverter.timeConverter("memory_", dataList));
    ...
    /**
     * 앱의 NetworkIO 사용량을 조회한다.
     *
     * @return Map
     */
    @RequestMapping(value = {"/app/getNetworkIoKByte"}, method = RequestMethod.POST)
    @ResponseBody
    public Map getAppMonitoringNetworkUsage(@RequestBody AppMonitoringChart appMonitoringChart,
                                            @RequestParam(value = "defaultTimeRange") String defaultTimeRange,
                                            @RequestParam(value = "groupBy") String groupBy) {
        ...
        if (appMonitoringChart.getIdx().toUpperCase().indexOf(",") >= 0) {
            ...
            for (String id : idx) {
                Map data = new HashMap();
                ResponseEntity<Map> result = commonService.procRestTemplate("/app/" + appMonitoringChart.getGuid() + "/" + id + "/networkIoKByte?defaultTimeRange=" + defaultTimeRange + "&groupBy=" + groupBy, HttpMethod.GET, null, null, Map.class);
                Map body = result.getBody();
            ...
            Map data = new HashMap();
            ResponseEntity<Map> result = commonService.procRestTemplate("/app/" + appMonitoringChart.getGuid() + "/" + appMonitoringChart.getIdx() + "/networkIoKByte?defaultTimeRange=" + defaultTimeRange + "&groupBy=" + groupBy, HttpMethod.GET, null, null, Map.class);

            Map body = result.getBody();
            data.put("idx", appMonitoringChart.getIdx());
            data.put("data", body);
            dataList.add(data);
        }
        ...
        List<Map> returnDataList = timeConverter.timeConvert(dataList);

        returnMap.put("data", returnDataList);
        return returnMap;
    }
}

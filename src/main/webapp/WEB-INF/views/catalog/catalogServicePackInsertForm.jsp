...
<div id="catalogFormView" style="display: none;">
    <div class="row" style="margin-top:-19px">
            ...
            <div class="col-sm-12 col-md-12 tar mt65">
                <div class="applist">
                    ...
                    <div class="name">
                        ...
                        <span id="catalogDescription"
                              style="width: 80%; color: #a2a2a2; line-height: 22px;"> desc </span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="panel content-box col-md-6 col-md-offset-13">
            ...
            <div id="serviceParameterform">
                <div class="col-sm-12 mt20">
                    <span style="font-size: 18px;">서비스 생성 파라미터 </span>
                </div>
                <div class="col-sm-12 mt10">
                    <div class="form-group has-default has-feedback">
                        <label class="control-label sr-only" for="serviceParameter"></label>
                        <div id="serviceParameterArea"></div>
                        <input type="hidden" class="form-control" id="serviceParameter" maxlength="100"
                               placeholder="(Optional) 서비스 파라미터 입력" value=""/>
                    </div>
                </div>
            </div>
            <div id="app_bind_yn_form">
                <div class="col-sm-12 mt20">
            ...
            <div id="appBindParameterform">
                <div class="col-sm-12 mt20">
                    <span style="font-size: 18px;">앱 바인드 파라미터 </span>
                </div>
                <div class="col-sm-12 mt10">
                    <div class="form-group has-default has-feedback">
                        <label class="control-label sr-only" for="appBindParameter"></label>
                        <div id="appBindParameterArea"></div>
                        <input type="hidden" class="form-control" id="appBindParameter" maxlength="100"
                               placeholder="(Optional) 앱 바인드 파라미터 입력" value=""/>
                    </div>
                </div>
            </div>
...
<sec:authorize access="isAuthenticated()">
    <input type="hidden" id="userId" name="userId" value="<sec:authentication property="details.userid" />"/>
</sec:authorize>
...
<script type="text/javascript">
    ...
    var procSetCommonInsertForm = function (data) {
        ...
        catalogDashboardUseYn = catalogList[0].dashboardUseYn;
        ...
        var service_arameter = catalogList[0].parameter;
        //서비스 생성 파라미터 Form 생성
        if(service_arameter != null) {
            parameterInsertForm(service_arameter, "serviceParameter", serviceParameterIds);
        }

        var app_bind_parameter = catalogList[0].app_bind_parameter;
        if(app_bind_parameter != null) {
            parameterInsertForm(app_bind_parameter, "appBindParameter", appBindParameterIds);
        }
        $("#appBindParameterform").hide();

        var appBindYn = catalogList[0].appBindYn;
        if (appBindYn == 'N') {
            $('#app_bind_yn_form').hide();
        } else {
            $('#app_bind_yn_form').show();
        }
    ...
    function parameterInsertForm(data,paramterName,parameterIds) {
        ...
        if (undefined == data || null == data || 'null' == data || '' == data) {
            ...
            $('#'+paramterName+"Area").html(htmlString);
        }
    }
    ...
    var procCatalogCfRun = function () {
        ...
        var catalogTypeParam = {
            ...
            app_bind_parameter: $('#appBindParameter').val(),
            userId: $('#userId').val(),
            dashboardUseYn: catalogDashboardUseYn
        };
...
//        var regExpPattern = /[\{\}\[\]\/?.,;:|\)*~`!^\-_+<>@\#$%&\\\=\(\'\"]/gi;
    ...
    $("#btnSave").on("click", function () {
        setParmeterData(serviceParameterIds,"serviceParameter");
        setParmeterData(appBindParameterIds,"appBindParameter");
        procCatalogCfRun();
    });

    function setParmeterData(parametersIds,paramterName){
        var data = "";

        for (var i = 0; i < parametersIds.length; i++) {
            var value = $("#"+paramterName+"_"+ parametersIds[i]).val();
            ...
            if (i > 0) {
                data = data + ',"' + parametersIds[i] + '":"' + value + '"';
            } else {
                data = data + '"' + parametersIds[i] + '":"' + value + '"';
            }
        }
    ...
    $("#appName").change(function () {
        ...
        if (undefined == reqAppName || null == reqAppName || 'null' == reqAppName || '' == reqAppName || '<%= Constants.NONE_VALUE %>' == reqAppName) {
            $("#appBindParameterform").hide("fast", function () {
            });
        } else {
            if(appBindParameterIds.length > 0){
                $("#appBindParameterform").show("fast", function () {
                });
            }

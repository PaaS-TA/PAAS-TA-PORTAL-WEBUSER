...
<script type="text/javascript">
    ...
    var procCallbackStarterRelationList = function (data) {
        ...
        for (var i = 0; i < servicePackListLength; i++) {
            reqArrayParam.push({
                ...
                app_bind_parameter: servicePackList[i].app_bind_parameter,
                appBindYn: servicePackList[i].appBindYn
            });
    ...
    var serviceIds = [];
    var serviceParameterIds = [];
    var appbindIds = [];
    var appBindParameterIds = [];
    ...
    var procCallbackMultiServicePlanList = function (data) {
        ...
        for (var i = 0; i < listCount; i++) {
            ...
            var app_bind_parameter = resultList[i].app_bind_parameter;
            var appBindYn = resultList[i].appBindYn;

            if (undefined == parameter || null == parameter || 'null' == parameter) parameter = '';
            if (undefined == app_bind_parameter || null == app_bind_parameter || 'null' == app_bind_parameter || '' == app_bind_parameter) app_bind_parameter = '';
            var serviceForm = '<div class="col-sm-9' + tempCss + '">'
                + '<span style="font-size:18px;">' + resultList[i].name + '</span></div>'
                + '<div class="col-sm-3 fr' + tempCss + '" style="height: 28px;">'
                + '<div class="fr"><div class="fl"><span style="color:#a2a2a2;" id="appBindYn_' + i + '_Form">앱 연결여부</span></div>'
                + '<div class="ml10 fl" id="appBindYn_check_' + i + '_Form">'
            ...
            if (!(undefined == parameter || null == parameter || 'null' == parameter || '' == parameter)) {
                serviceForm += '<div class="col-sm-12 mt10" id="serviceParameter_' + i + '_Form"><span style="font-size:18px;"> 서비스 생성 파라미터 </span></div>'
                    ...
                    + '<div id="serviceParameter_' + i + '_Area"></div>'
                    + '<input type="hidden" class="form-control" id="serviceParameter_' + i + '" maxlength="100" '
                    + 'placeholder="(Optional) 서비스 파라미터 입력" value=""></div></div>';
            }
            serviceForm += '<div id="appBindParameter_' + i + '_Form"><div class="col-sm-12 mt10"><span style="font-size:18px;"> 앱 바인드 파라미터 </span></div>'
                + '<div class="col-sm-12 mt10"><div class="form-group has-default has-feedback">'
                + '<label class="control-label sr-only" for="appBindParameter_' + i + '"></label>'
                + '<div id="appBindParameter_' + i + '_Area"></div>'
                + '<input type="hidden" class="form-control" id="appBindParameter_' + i + '" maxlength="100" '
                + 'placeholder="(Optional) 앱 바인드 파라미터 입력" value=""></div></div></div>';

            serviceForm += '<div class="col-sm-12 mt10"><span style="font-size:18px;"> 서비스 이용사양 선택 </span></div>'
                + '<div class="col-sm-12 pt5">'
            ...
            htmlString.push(serviceForm);
            objDiv.append(htmlString);
            if(parameter != null){
                parameterInsertForm(parameter, "serviceParameter_" + i, serviceParameterIds);
            }
            if(app_bind_parameter != null) {
                parameterInsertForm(app_bind_parameter, "appBindParameter_" + i, appBindParameterIds);
            }
            serviceIds.push("serviceParameter_" + i);
            appbindIds.push("appBindParameter_" + i);
            if (appBindYn == "N") {
                $("#appBindYn_" + i + "_Form").hide();
                $("#appBindYn_check_" + i + "_Form").hide();
            }
        ...
        for (var j = 0; j < listCount; j++) {
            ...
            if ($("#appBindParameter_" + j).val().length <= 0) {
                $("#app_bind_parameter_" + j + "_Form").hide();
            }
        }
        ...
        $(".checkBoxAppBindYn").on("change", function () {
            ...
            if ($(this).prop('checked')) {
                ...
                if(appBindParameterIds.length > 0 ){
                    $("#appBindParameter_" + tempArray[1] + "_Form").show("fast", function () {
                    });
                }
            } else {
                appBindYn.val('<%= Constants.USE_YN_N %>');
                $("#appBindParameter_" + tempArray[1] + "_Form").hide("fast", function () {
                });
            }
        }).bootstrapToggle({
            on: 'ON',
    ...
    function setParmeterData(ids, paramters) {
        for (var idNum = 0; idNum < ids.length; idNum++) {
            var data = "";
            for (var i = 0; i < paramters.length; i++) {
                var value = $("#" + paramters[i]).val();
                var sParam = paramters[i].split("_");
                var id = sParam[0] + "_" + sParam[1];
                if (ids[idNum] == id) {
                    if (value == undefined || value == null || value == 'null' || value == 'defalut') {
                        value = "default";
                    }

                    var parameterId = paramters[i].replace(ids[idNum] + "_", "");
                    if (data.length > 0) {
                        data = data + ',"' + parameterId + '":"' + value + '"';
                    } else {
                        data = data + '"' + parameterId + '":"' + value + '"';
                    }
                }
    ...
    function parameterInsertForm(data, paramterName, parameterIds) {
        ...
        for (var i = 0; i < split.length; i++) {
            ...
            if (splitSign != null && splitSign != "undefined" && splitSign != "") {
                ...
                if (splitSign[1].trim() == "defalut") {
                    htmlString.push(splitSign[0] + '  <input type="hidden" class="form-control" id="' + paramterName + '_' + splitSign[0] + '" maxlength="60"  value="defalut" /><br>');
                }

                parameterIds.push(paramterName + '_' + splitSign[0]);
            }
        }

        if (htmlString.length == 0) {
            $('#' + paramterName + '_Form').hide();
        }

        // 5. 파라미터가 존재할 때 서비스 생성 파라미터 input box 가 보임.
        if (undefined == data || null == data || 'null' == data || '' == data) {
            $('#' + paramterName + "_Form").hide();
        } else {
            $('#' + paramterName + "_Area").html(htmlString);
        }
    }
    ...
    var procCatalogCfRun = function () {
        ...
        for (var j = 0; j < listCount; j++) {
            servicePlanList.push({
                ...
                app_bind_parameter: $('#appBindParameter_' + j).val()
            });
        }

        var catalogTypeParam = {
            hostName: hostNameObject.val(),
...
//        var regExpPattern = /[\{\}\[\]\/?.,;:|\)*~`!^\-_+<>@\#$%&\\\=\(\'\"]/gi;
    ...
    $("#btnSave").on("click", function () {
        setParmeterData(serviceIds, serviceParameterIds);
        setParmeterData(appbindIds, appBindParameterIds);
